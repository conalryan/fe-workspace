...
export class CustomizingEventListenerOptions extends LitElement {
   ...
   * - the browser knows that the event handler won't try to prevent the default behavior of the event (like __scrolling__ for __touch__ events)
   * - This allows the browser to immediately begin processing the event (like __scrolling__) without waiting for your event handler to complete,
   *    resulting in smoother scrolling and better performance
   * - This is particularly important for __touch__ and __wheel__ events on mobile devices, where scrolling performance is crucial
   * ```ts
